...
describe("generateTableSchema", () => {
	beforeEach(() => {
		...
		getTableInfoMock.mockResolvedValue({
			fields: {
				name: "DEFINE FIELD name ON command TYPE string PERMISSIONS FULL",
			},
		});
	});
	...
	it("generates a schema for a SCHEMAFULL table", async () => {
		...
		expect(inputFields).toBe(
			"const testInputSchemaGen = z.object({\nname: z.string()\n})",
		);

		expect(outputFields).toBe(
			"const testOutputSchemaGen = z.object({\nname: z.string()\n})",
		);
	});
