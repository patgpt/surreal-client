...
const handleStringAssertions = (schema: string, condition: string): string => {
	...
	if (
		processedCondition.includes("NOT IN") ||
		processedCondition.includes("NOTINSIDE")
	) {
		const notInMatch = processedCondition.match(
			/(NOT IN|NOTINSIDE)\s*(\[.*?])/,
		);
		if (notInMatch?.[2]) {
...
const handleArrayAssertions = (schema: string, condition: string): string => {
	...
	if (condition.includes("array::len($value)")) {
		const lenMatch = condition.match(
			/array::len\(\$value\)\s*(>|>=|=|<=|<)\s*(\d+)/,
		);
		if (lenMatch) {
