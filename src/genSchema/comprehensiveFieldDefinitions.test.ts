...
describe("Comprehensive Field Definition Tests", () => {
		...
		it("should generate valid TypeScript property names for special characters", () => {
			const fields = [
				getDetailsFromDefinition(
					"DEFINE FIELD `user-id` ON TABLE data TYPE string;",
					false,
				),
				getDetailsFromDefinition(
					"DEFINE FIELD `full name` ON TABLE data TYPE string;",
					false,
				),
				getDetailsFromDefinition(
					"DEFINE FIELD `123abc` ON TABLE data TYPE string;",
					false,
				),
			];
	...
	describe("Recipe table production cases", () => {
		...
		it("should handle option<array> types correctly", () => {
			...
			expect(result.zodString).toBe(
				"recordId('chef_subscription_tier').array().optional()",
			);
		});
	});
	...
	describe("Default value generation in schemas", () => {
		...
		it("should handle numeric default values", () => {
			const fields = [
				getDetailsFromDefinition(
					"DEFINE FIELD count ON TABLE stats TYPE number DEFAULT 0;",
					false,
				),
			];
		...
		it("should not add default to optional fields", () => {
			const fields = [
				getDetailsFromDefinition(
					'DEFINE FIELD bio ON TABLE user TYPE string DEFAULT "No bio";',
					true,
				),
			];
			const schema = generateZodSchemaCode(fields, "userSchema");
