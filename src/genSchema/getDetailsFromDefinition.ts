...
export type FieldDetail = TokenizedDefinition & {
	zodString: string;
	skip: boolean;
};
...
const makeOptional = (
	...
	const typeIsOptional =
		tokens.type?.toLowerCase().startsWith("option<") ?? false;
	if (
...
const makeFlexible = (zodString: string, isFlexible: boolean) =>
	isFlexible ? `${zodString}.passthrough()` : zodString;
...
export const getZodTypeFromQLType = (
	...
	if (
		typeStringToProcess.toLowerCase().startsWith("option<") &&
		typeStringToProcess.endsWith(">")
	) {
		isExplicitlyOptional = true;
		typeStringToProcess = typeStringToProcess.substring(
			"option<".length,
			typeStringToProcess.length - 1,
		);
	}

	if (typeStringToProcess.includes("|")) {
		const unionParts = typeStringToProcess
			.split("|")
			.map((part) => part.trim());
		...
		const allKnownTypes = unionParts.every((part) => {
			...
			return typeKeyword
				? KNOWN_TYPES.includes(
						typeKeyword.toLowerCase() as (typeof KNOWN_TYPES)[number],
					)
				: false;
		});
		...
		if (!allKnownTypes) {
			const allUnquotedIdentifiers = unionParts.every((part) =>
	...
	if (
		typeStringToProcess.startsWith("range<") &&
		typeStringToProcess.endsWith(">")
	) {
	...
	if (typeStringToProcess) {
		const valueExtractionRegex =
			/"([^"\\]*(?:\\.[^"\\]*)*)"|'([^'\\]*(?:\\.[^'\\]*)*)'/g;
		const valueMatchesForValidation = Array.from(
			typeStringToProcess.matchAll(valueExtractionRegex),
		);
		...
		if (valueMatchesForValidation.length > 0) {
			...
			const normalizedOriginal = typeStringToProcess.replace(
				/\s*\|\s*/g,
				" | ",
			);
			const normalizedReconstructed = reconstructedForValidation.replace(
				/\s*\|\s*/g,
				" | ",
			);
	...
	if (match?.[1]) {
		...
		const currentLevelTokens: TokenizedDefinition = {
			...tokens,
			type: typeStringToProcess,
		};
		let schema = getSchemaForType(
			typeKeyword,
			currentLevelTokens,
			isInputSchema,
			subSchema,
...
export const shouldFieldBeSkipped = (
	tokens: TokenizedDefinition,
	isInputSchema: boolean,
): boolean => {
	return (
		!!tokens.value?.trim().toLowerCase().startsWith("<future>") && isInputSchema
	);
};

export const getDetailsFromDefinition = (
	definition: string,
	isInputSchema: boolean,
): FieldDetail => {
