...
export const generateSchemaForTable = async (
	name: string,
	tableInfo: string,
) => {
...
export const generateTableSchema = async (
	...
	try {
		...
		for (const name in tableInfo) {
			...
			const genSchemaFileName = resolve(
				tableSchemaFolder,
				`${toCamelCase(tableName)}SchemaGen.ts`,
			);

			const injectRecordSchema =
				inputFields.includes("recordId(") || outputFields.includes("recordId(");
