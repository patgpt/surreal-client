...
const PATTERNS = {
	...
	CAPTURE_UNTIL_NEXT_CLAUSE: (keyword: string) =>
		new RegExp(
			`${keyword}\\s+(.*?)(?=${`\\s+(?:${FIELD_CLAUSES.join("|")})\\s+|$`})`,
			"im",
		),
	FOR_CLAUSE:
		/FOR\s+(select|create|update|delete)\s+([^FOR]*?)(?=\s+FOR\s+|$)/gim,
	DEFINE_FIELD:
		/DEFINE FIELD(?: IF NOT EXISTS)?\s+((?:`[^`]+`|[^`])+?)\s+ON(?: TABLE)?\s+([\w.:`\-[\]*]+)/im,
} as const;
...
function parsePermissions(
	...
	const nextClauseActualIndex = findNextClause(
		definition,
		permIndex + ` ${permKeyword} `.length,
	);
	const permSection =
		nextClauseActualIndex === -1
			? definition.slice(permIndex)
			: definition.slice(permIndex, nextClauseActualIndex);

	const permissions: Permissions = {};
	const remainingDef =
		definition.slice(0, permIndex) +
		(nextClauseActualIndex !== -1
			? definition.slice(nextClauseActualIndex)
			: "");
	...
	for (const match of forMatches) {
		...
		if (operation && expression) {
			permissions[operation.toLowerCase() as PermissionOperation] =
				expression.trim();
		}
	}
	return [
...
export const tokenize = (originalDefinition: string): TokenizedDefinition => {
	...
	if (nameMatch) {
		...
		remainingDefinitionAfterNameAndTable = definition
			.replace(defineFieldPartRegex, "")
			.trim();
	}
	...
	const flexibleTypeIndex = currentDefinitionSegment
		.toUpperCase()
		.indexOf(flexibleTypeKeyword);
	const simpleTypeIndex = currentDefinitionSegment
		.toUpperCase()
		.indexOf(typeKeyword);

	if (
		flexibleTypeIndex !== -1 &&
		(simpleTypeIndex === -1 || flexibleTypeIndex < simpleTypeIndex)
	) {
	...
	const defaultMatch = currentDefinitionSegment.match(
		PATTERNS.CAPTURE_UNTIL_NEXT_CLAUSE("DEFAULT"),
	);
	if (defaultMatch?.[1]) {
		const defaultValue = defaultMatch[1].trim().replace(/;$/, "").trim();
		const readonlyInDefaultMatch = defaultValue
			.toUpperCase()
			.match(/(.*)\s+READONLY$/);
	...
	const valueMatch = currentDefinitionSegment.match(
		PATTERNS.CAPTURE_UNTIL_NEXT_CLAUSE("VALUE"),
	);
	if (valueMatch?.[1]) {
		result.value = valueMatch[1].trim().replace(/;$/, "").trim();
	}

	const assertMatch = currentDefinitionSegment.match(
		PATTERNS.CAPTURE_UNTIL_NEXT_CLAUSE("ASSERT"),
	);
	if (assertMatch?.[1]) {
		result.assert = assertMatch[1].trim().replace(/;$/, "").trim();
	}

	const referenceMatch = currentDefinitionSegment.match(
		PATTERNS.CAPTURE_UNTIL_NEXT_CLAUSE("REFERENCE"),
	);
	if (referenceMatch?.[1]) {
		result.reference = referenceMatch[1].trim().replace(/;$/, "").trim();
	}

	const commentMatch = currentDefinitionSegment.match(
		PATTERNS.CAPTURE_UNTIL_NEXT_CLAUSE("COMMENT"),
	);
	if (commentMatch?.[1]) {
