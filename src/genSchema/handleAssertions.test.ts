...
describe("handleAssertions", () => {
	describe("String assertions", () => {
		it("handles string::is:: assertions", () => {
			expect(
				handleAssertions("z.string()", "string::is::email()", "string"),
			).toBe("z.string().email()");
			expect(
				handleAssertions("z.string()", "string::is::url()", "string"),
			).toBe("z.string().url()");
			expect(
				handleAssertions("z.string()", "string::is::uuid()", "string"),
			).toBe("z.string().uuid()");
			expect(
				handleAssertions("z.string()", "string::is::ip()", "string"),
			).toContain("Invalid IP address");
			expect(
				handleAssertions("z.string()", "string::is::ipv4()", "string"),
			).toContain("Invalid IPv4 address");
			expect(
				handleAssertions("z.string()", "string::is::ipv6()", "string"),
			).toContain("Invalid IPv6 address");
			expect(
				handleAssertions("z.string()", "string::is::datetime()", "string"),
			).toBe("z.string().datetime()");
		});

		it("handles regex-based string::is:: assertions", () => {
			expect(
				handleAssertions("z.string()", "string::is::alphanum()", "string"),
			).toBe("z.string().regex(/^[a-zA-Z0-9]*$/)");
			expect(
				handleAssertions("z.string()", "string::is::alpha()", "string"),
			).toBe("z.string().regex(/^[a-zA-Z]*$/)");
			expect(
				handleAssertions("z.string()", "string::is::ascii()", "string"),
			).toBe("z.string().regex(/^[\x00-\x7F]*$/)");
			expect(
				handleAssertions("z.string()", "string::is::hexadecimal()", "string"),
			).toBe("z.string().regex(/^[0-9a-fA-F]*$/)");
			expect(
				handleAssertions("z.string()", "string::is::latitude()", "string"),
			).toBe(
				"z.string().regex(/^(+|-)?(?:90(?:(?:.0{1,6})?)|(?:[0-9]|[1-8][0-9])(?:(?:.[0-9]{1,6})?))$/)",
			);
			expect(
				handleAssertions("z.string()", "string::is::longitude()", "string"),
			).toBe(
				"z.string().regex(/^(+|-)?(?:180(?:(?:.0{1,6})?)|(?:[0-9]|[1-9][0-9]|1[0-7][0-9])(?:(?:.[0-9]{1,6})?))$/)",
			);
			expect(
				handleAssertions("z.string()", "string::is::numeric()", "string"),
			).toBe("z.string().regex(/^[0-9]*$/)");
			expect(
				handleAssertions("z.string()", "string::is::semver()", "string"),
			).toBe(
		...
		it("handles string length comparisons", () => {
			expect(
				handleAssertions("z.string()", "string::len($value) > 5", "string"),
			).toBe("z.string().min(6)");
			expect(
				handleAssertions("z.string()", "string::len($value) >= 5", "string"),
			).toBe("z.string().min(5)");
			expect(
				handleAssertions("z.string()", "string::len($value) < 10", "string"),
			).toBe("z.string().max(9)");
			expect(
				handleAssertions("z.string()", "string::len($value) <= 10", "string"),
			).toBe("z.string().max(10)");
			expect(
				handleAssertions("z.string()", "string::len($value) = 8", "string"),
			).toBe("z.string().length(8)");
		});

		it("handles IN and INSIDE assertions", () => {
			expect(
				handleAssertions("z.string()", "IN ['a', 'b', 'c']", "string"),
			).toBe("z.enum(['a', 'b', 'c'])");
			expect(
				handleAssertions("z.string()", "INSIDE ['x', 'y', 'z']", "string"),
			).toBe("z.enum(['x', 'y', 'z'])");
		});
		...
		it("handles NOT IN and NOTINSIDE assertions", () => {
            ...
            `;
			expect(
				handleAssertions("z.string()", "NOT IN ['a', 'b', 'c']", "string"),
			).toEqualIgnoringWhitespace(expected);
			expect(
				handleAssertions("z.string()", "NOTINSIDE ['a', 'b', 'c']", "string"),
			).toEqualIgnoringWhitespace(expected);
		});
		...
		it("handles word count assertions", () => {
			...
			expect(
				handleAssertions(
					"z.string()",
					"array::len(string::words($value)) > 1",
					"string",
				),
		...
		it("handles conditional word count assertions", () => {
			...
			expect(
				handleAssertions(
					"z.string()",
					"IF $value THEN array::len(string::words($value)) > 1 ELSE true END",
					"string",
				),
		...
		it("handles unknown string::is:: assertion", () => {
			expect(
				handleAssertions("z.string()", "string::is::unknown()", "string"),
			).toBe("z.string()");
		});

		describe("Number assertions", () => {
			it("handles comparison assertions", () => {
				expect(handleAssertions("z.number()", "> 0", "number")).toBe(
					"z.number().min(1)",
				);
				expect(handleAssertions("z.number()", "<= 100", "number")).toBe(
					"z.number().max(100)",
				);
				expect(handleAssertions("z.number()", "= 50", "number")).toBe(
					"z.literal(50)",
				);
				expect(handleAssertions("z.number()", "== 90", "number")).toBe(
					"z.literal(90)",
				);
			});
			...
			it("handles IN/INSIDE numeric enum assertions", () => {
				...
				expect(handleAssertions("z.number()", "INSIDE [10]", "number")).toBe(
					"z.literal(10)",
				);
			});
			...
			it("filters NONE/NULL and preserves numeric literals in numeric enum assertions", () => {
				expect(
					handleAssertions(
						"z.number()",
						"IN [NONE, NULL, 1, 2.5, -3]",
						"number",
					),
				).toBe("z.union([z.literal(1), z.literal(2.5), z.literal(-3)])");
			});
		});
		...
		describe("Date assertions", () => {
			it("handles empty assertion", () => {
				expect(
					handleAssertions("z.date()", "", "date"),
				).toEqualIgnoringWhitespace("z.date()");
			});
			it("handles any assertion", () => {
				expect(
					handleAssertions("z.date()", "> time::now() + 30d", "date"),
				).toEqualIgnoringWhitespace("z.date()");
			});
		});
		...
		describe("Array assertions", () => {
			it("handles array length assertions", () => {
				...
				).toEqualIgnoringWhitespace(lengthAssert);
			});
			...
			it("throws error for invalid ALLINSIDE assertions", () => {
				expect(() =>
					handleAssertions("z.array(z.unknown())", "ALLINSIDE", "array"),
				).toThrow("Invalid ALLINSIDE assertion");
			});
		});
		...
		describe("Multiple assertions", () => {
			it("handles multiple assertions combined with AND", () => {
				...
				expect(
					handleAssertions(
						"z.string()",
						"string::len($value) > 0 AND string::is::email()",
						"string",
					),
				).toBe(multipleAssert);
			});
		...
		describe("Edge cases", () => {
			...
			it("handles unknown schema types", () => {
				expect(
					handleAssertions("z.unknown()", "some assertion", "unknown"),
				).toBe("z.unknown()");
			});

			it("handles unkown assertion", () => {
				expect(handleAssertions("z.string()", "unknown", "string")).toBe(
					"z.string()",
				);
			});
		});
		...
		describe("Bug fixes", () => {
			...
			it("handles number assertions with trailing parentheses from split conditions", () => {
				...
				expect(handleAssertions("z.number()", "$value <= 100)", "number")).toBe(
					"z.number().max(100)",
				);
				expect(handleAssertions("z.number()", "$value >= 0)", "number")).toBe(
					"z.number().min(0)",
				);
			});
			...
			it("handles assertion with $value = NONE OR prefix for numbers", () => {
				...
				expect(
					handleAssertions(
						"z.number()",
						"$value = NONE OR ($value >= 0 AND $value <= 100)",
						"number",
					),
				).toBe("z.number().min(0).max(100)");
			});
			...
			it("strips $value = NONE OR prefix from string enum assertions", () => {
				expect(
					handleAssertions(
						"z.string()",
						"$value = NONE OR $value IN ['ebook', 'video', 'audio']",
						"string",
					),
				).toBe("z.enum(['ebook', 'video', 'audio'])");
			});
			...
			it("handles INSIDE with $value = NONE OR prefix", () => {
				expect(
					handleAssertions(
						"z.string()",
						"$value = NONE OR $value INSIDE ['pending', 'done']",
						"string",
					),
				).toBe("z.enum(['pending', 'done'])");
			});
			...
			it("handles multiple NONE/NULL prefixes in string enum assertions", () => {
				expect(
					handleAssertions(
						"z.string()",
						"$value = NONE OR $value = NULL OR $value IN ['open', 'closed']",
						"string",
					),
			...
			it("handles multiple NONE/NULL prefixes in numeric enum assertions", () => {
				expect(
					handleAssertions(
						"z.number()",
						"$value = NONE OR $value = NULL OR $value IN [1, 2, 3]",
						"number",
					),
				).toBe("z.union([z.literal(1), z.literal(2), z.literal(3)])");
			});
		});
	});
});
