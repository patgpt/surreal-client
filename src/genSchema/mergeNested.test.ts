...
describe("mergeNested", () => {
	it("correctly processes simple fields", () => {
		const fields: Record<string, string> = {
			...
			published:
				"DEFINE FIELD published ON book TYPE datetime PERMISSIONS FULL",
		};
	...
	it("correctly processes nested objects and arrays", () => {
		const fields: Record<string, string> = {
			...
			published:
				"DEFINE FIELD published ON book TYPE datetime PERMISSIONS FULL",
			vendors:
				"DEFINE FIELD vendors ON book TYPE option<array<object>> PERMISSIONS FULL",
			"vendors[*]":
				"DEFINE FIELD vendors[*] ON book TYPE object PERMISSIONS FULL",
			"vendors[*].name":
				"DEFINE FIELD vendors[*].name ON book TYPE string PERMISSIONS FULL",
			"vendors[*].price":
				"DEFINE FIELD vendors[*].price ON book TYPE number PERMISSIONS FULL",
			"vendors[*].ratings":
				"DEFINE FIELD vendors[*].ratings ON book TYPE array<object> DEFAULT [] PERMISSIONS FULL",
			"vendors[*].ratings[*]":
				"DEFINE FIELD vendors[*].ratings[*] ON book TYPE object PERMISSIONS FULL",
	...
	it("correctly handles fields with VALUE <future>", () => {
		const fields: Record<string, string> = {
			...
			skipped:
				"DEFINE FIELD skipped ON book TYPE number VALUE <future> { 1 + 1 } PERMISSIONS FULL",
