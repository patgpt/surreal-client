...
import { cancel, confirm, group, intro, isCancel, log, note, password, text } from '@clack/prompts'
...
export async function runWizard(opts: WizardOptions = {}): Promise<Config & { configPath: string }> {
	...
	intro(chalk.cyan('◆ ') + chalk.bold('SurrealDB Codegen') + chalk.dim(' — configuration wizard'))
	...
	if (hasExisting) {
		...
		if (useCached) {
			log.success(chalk.hex('#00ff41')('Loaded config from ') + chalk.dim(cfgPath))
			return {
	...
	const connection = await group(
		{
			surreal: () =>
				text({
					...
					defaultValue: flags.surreal ?? existingConfig.surreal ?? DEFAULT_ENDPOINT,
					validate: v => {
						if (v && !/^https?:\/\/.+/.test(v)) return 'Must be a valid HTTP/S URL'
					},
				}),
			ns: () =>
				text({
			...
			username: () =>
				text({
					...
					defaultValue: flags.username ?? existingConfig.username ?? DEFAULT_USER,
				}),
			password: () =>
				password({
	...
	const output = await group(
		{
			outputFolder: () =>
				text({
					...
					defaultValue: flags.outputFolder ?? existingConfig.outputFolder ?? DEFAULT_OUTPUT,
				}),
			schemaFile: () =>
				text({
					message: chalk.cyan('Schema file path') + chalk.dim(' (leave blank to connect live)'),
			...
			generateClient: () =>
				confirm({
					...
					initialValue: flags.generateClient ?? existingConfig.generateClient ?? true,
				}),
			surrealImage: () =>
				text({
					message: chalk.cyan('SurrealDB Docker image'),
					placeholder: DEFAULT_IMAGE,
					defaultValue: flags.surrealImage ?? existingConfig.surrealImage ?? DEFAULT_IMAGE,
				}),
		},
		{ onCancel: () => handleCancel(null) },
	)
