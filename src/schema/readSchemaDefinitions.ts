...
const collectSchemaFiles = async (
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			files.push(
				...(await collectSchemaFiles(baseDirectory, fullPath, ignoreRules)),
			);
			continue;
		}
...
export const readSchemaDefinitions = async (
	schemaPath: string,
): Promise<string> => {
	...
	if (!schemaStats.isDirectory()) {
		throw new Error(
			`Schema path is neither file nor directory: ${resolvedPath}`,
		);
	}
	...
	const fileContents = await Promise.all(
		sortedFiles.map((file) => readFile(file, "utf-8")),
	);
