...
describe("readSchemaDefinitions", () => {
	...
	it("reads schema files recursively from a directory", async () => {
		...
		await writeFile(
			resolve(tempDir, "nested", "b.surql"),
	...
	it("respects .ignore patterns", async () => {
		...
		await writeFile(
			resolve(tempDir, ".ignore"),
			"ignored/**\nexcluded.surql\n",
		);
		await writeFile(
			resolve(tempDir, "ignored", "skip.surql"),
			"DEFINE TABLE skip SCHEMAFULL;",
		);
		await writeFile(
			resolve(tempDir, "excluded.surql"),
			"DEFINE TABLE excluded SCHEMAFULL;",
		);
		await writeFile(
			resolve(tempDir, "included", "keep.surql"),
	...
	it("ignores non schema files", async () => {
		tempDir = await mkdtemp(resolve(tmpdir(), "surql-gen-schema-"));
		await writeFile(
			resolve(tempDir, "schema.txt"),
			"DEFINE TABLE nope SCHEMAFULL;",
		);
		await writeFile(
			resolve(tempDir, "schema.surql"),
			"DEFINE TABLE yep SCHEMAFULL;",
		);
	...
	it("throws if directory has no schema files", async () => {
		...
		await expect(readSchemaDefinitions(tempDir)).rejects.toThrow(
